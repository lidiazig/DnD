...
class HechizosController {
    ...
    public async getAllOneClass(req: Request, res: Response){
        const { id } = req.params;
        const hechizos = await pool.then((r:any) => r.query('select h.id as id, h.nombre as nombre, hc.nivel as nivel from hechizos h inner join hechizos_clase hc on hc.id_hechizo=h.id where hc.id_clase=?',[id]));
